...
def notify(title: str, message: str):
    ...
    if topic:
        topic = topic.strip()  # guard against stray spaces/newlines in the secret
        ...
        try:
            ...
            # Log the actual result so failures aren't silent.
            print(f"[ntfy] POST {url} -> HTTP {resp.status_code}")
            if resp.status_code >= 400:
                print(f"[ntfy error] {resp.text[:300]}", file=sys.stderr)
            else:
                sent = True
        except requests.RequestException as e:
            print(f"[ntfy error] {e}", file=sys.stderr)
    else:
        print("[ntfy] NTFY_TOPIC is not set")
...
def run(html=None):
    # Set TEST_PING=1 to force a notification immediately (ignores dates/state).
    if os.environ.get("TEST_PING"):
        notify("MSF watcher test", "If this reaches your phone, the pipe works.")
        return {"test": True}
